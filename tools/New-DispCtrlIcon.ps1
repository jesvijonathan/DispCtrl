<#
.SYNOPSIS
    Generates DispCtrl's application icon.

.DESCRIPTION
    Draws a monitor with one dark panel and one lit, which is what DispCtrl is
...
#>
param(
    [string]$OutputPath = (Join-Path (Split-Path -Parent (Split-Path -Parent $MyInvocation.MyCommand.Path)) 'src\DispCtrl.App\Assets\DispCtrl.ico')
)
...
# Outer body. Near-black, matching the OLED case DispCtrl exists for.
