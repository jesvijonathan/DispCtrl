...
- **The icon's right-click menu**: Quick panel, Open DispCtrl, Simple view
  (`QuickPanel.Simple`, the title bar's dot), Put every display back
  (`RestoreVisibility`, the hotkey's twin), Keep on the taskbar, Hide this
  icon, Stop the engine, Exit DispCtrl. Stop sets `Local\DispCtrl.Engine.Stop`,
  never ends the process - the engine must unwind to put taskbars back. Exit
  first sets `Local\DispCtrl.App.Quit` (`QuickPanelSignal.RequestQuit`), which
  the app's listener turns into a flushed save and `Exit()`, then stops the engine.
