...
$compiler = if ($env:DISPLCTRL_CXX) { $env:DISPLCTRL_CXX } elseif (Get-Command g++.exe -ErrorAction SilentlyContinue) { (Get-Command g++.exe).Source } else { 'C:\toolchains\mingw64\bin\g++.exe' }
if (-not (Test-Path -LiteralPath $compiler)) { throw 'Taskbar glass requires an x64 MinGW-w64 C++ compiler. Set DISPLCTRL_CXX to g++.exe.' }
...
$target = Join-Path $OutputDirectory "DisplCtrl.TaskbarGlass.$revision.dll"
if (-not (Test-Path -LiteralPath $target)) {
    & $compiler '-std=c++20' '-O2' '-shared' '-static' '-static-libgcc' '-static-libstdc++' '-DUNICODE' '-D_UNICODE' $source '-o' $target '-lole32' '-loleaut32' '-lruntimeobject' '-luuid' '-ld2d1' '-Wl,--no-insert-timestamp'
