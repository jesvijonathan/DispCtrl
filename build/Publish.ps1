...
param(
    ...
    [switch]$KeepOld,
    # Both by default. x64 is the installer, the MSIX and the zips; ARM64 is
    # zips only, for Windows on ARM (Snapdragon laptops, a Raspberry Pi).
...
try {
    ...
    foreach ($arch in $Architectures) {
        # x64 keeps the folder names the installer, the MSIX and the workflows
        # read; ARM64 goes beside it with a suffix, and only as zips for now.
        $suffix = if ($arch -eq 'x64') { '' } else { "-$arch" }
        $rid = "win-$arch"
        $cli = Join-Path $artifactRoot "cli$suffix"
        $desktop = Join-Path $artifactRoot "desktop$suffix"
        ...
        foreach ($project in @('DispCtrl.Cli','DispCtrl.Engine')) {
            & dotnet publish "src/$project/$project.csproj" -c Release -r $rid --self-contained true -p:PublishAot=false -p:PublishTrimmed=false -p:PublishReadyToRun=true -p:Version=$Version -o $cli
        ...
        & dotnet publish src/DispCtrl.App/DispCtrl.App.csproj -c Release -r $rid --self-contained true -p:WindowsAppSDKSelfContained=true -p:PublishAot=false -p:PublishTrimmed=false -p:PublishReadyToRun=true -p:Version=$Version -o $desktop
        ...
        # Skipped on CI runners, which may have no interactive desktop, and for an
        # architecture this machine does not run natively: an x64 PC cannot start
        # an ARM64 build, and an ARM one would run the x64 build emulated.
        if (-not $env:CI -and $arch -eq $hostArch) {
        ...
        foreach ($folder in @($cli,$desktop)) {
            ...
            $manifest = [ordered]@{ product='DispCtrl'; version=$Version; channel=$Channel; architecture=$arch; commit=$commit; modifiedSources=$modified; builtUtc=[DateTimeOffset]::UtcNow.ToString('O'); nativeAot=$false; readyToRun=$true }
        ...
        foreach ($kind in @('cli','desktop')) {
            Compress-Archive -Path (Join-Path $artifactRoot "$kind$suffix/*") -DestinationPath (Join-Path $artifactRoot "DispCtrl-$Version-$Channel-$rid-$kind.zip") -CompressionLevel Optimal
        }
    }
