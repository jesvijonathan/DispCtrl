...
public sealed partial class MainViewModel
{
    ...
    public double OledIdleDim { get => Care.DimPercent; set { int v = Number(value, 0, 100); if (Care.DimPercent == v) return; Care.DimPercent = v; SaveProtection(); } }
    ...
    public double OledRestMinutes { get => Care.RestMinutes; set { int v = Number(value, 1, 30); if (Care.RestMinutes == v) return; Care.RestMinutes = v; SaveProtection(); } }
    public double TaskbarOpacity { get => _settings.Global.TaskbarOpacity; set { int v = Number(value, 30, 100); if (_settings.Global.TaskbarOpacity == v) return; _settings.Global.TaskbarOpacity = v; SaveProtection(); } }
    ...
    private void RaiseProtectionSettings()
    {
        foreach (string name in new[] { nameof(FocusEnabled), nameof(FocusDim), nameof(FocusDelay), nameof(FocusFade),
            nameof(FocusOledOnly), nameof(WindowTransition), nameof(FocusPerMonitor), nameof(FocusKeepHoveredClear), nameof(FocusScaleWithBrightness), nameof(FocusFollowMouse), nameof(FocusOtherMonitors), nameof(FocusPauseFullscreen), nameof(FocusKeepTaskbar),
            nameof(FocusExcludedApps), nameof(OledIdleEnabled), nameof(OledIdleMinutes), nameof(OledIdleDim), nameof(OledIdleFade),
            nameof(OledPauseFullscreen), nameof(OledRestMinutes), nameof(TaskbarOpacity), nameof(WindowsTransparency),
