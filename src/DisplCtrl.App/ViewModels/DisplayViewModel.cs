...
public sealed class DisplayViewModel : INotifyPropertyChanged
{
    ...
    private readonly DisplCtrlSettings _root;
    ...
    public DisplayViewModel(DisplayInfo display, MonitorSettings settings, DisplCtrlSettings root,
    ...
    /// </summary>
    /// <remarks>
    /// DisplCtrl cannot move the primary taskbar, but Windows can — so rather than
    ...
    /// DisplCtrl's own parking, which overrides the sliver Explorer leaves.
    /// </remarks>
    ...
    public string HideTaskbarDescription => _display.IsPrimary
        ? "DisplCtrl cannot move the primary taskbar — Explorer restores it immediately. This uses Windows' own auto-hide instead, which does work here but leaves a one-pixel lit strip along the edge."
    ...
    /// support call, and because DisplCtrl's own identity token is built from it —
    ...
    /// resolution: when DisplCtrl reclaims a hidden taskbar's strip, this is the
    ...
    /// lead, with the taskbar noted only when DisplCtrl is actually managing it.
    /// </remarks>
    ...
    /// <summary>Returns this display's DisplCtrl settings to their defaults.</summary>
    /// <remarks>
    /// Deliberately does not touch resolution, refresh rate, HDR or scaling.
    /// Those belong to Windows, not to DisplCtrl, and silently rewriting them would
    /// be far more destructive than the word "reset" implies.
    /// </remarks>
    /// <summary>
    /// Puts this display back to defaults — DisplCtrl's, and the monitor's own.
    /// </summary>
    /// <remarks>
    /// Resetting DisplCtrl's settings alone looked like a button that did nothing,
    ...
    /// deliberately left alone: they are not DisplCtrl's to reset, and changing them
    ...
    /// backlight DisplCtrl can reach, dimming in software instead would be strictly
    ...
    public string RotationDetail =>
        ...
        + "administrator rights once \u2014 the rest of DisplCtrl never needs them.";
    ...
    /// </summary>
    /// <remarks>
    /// Named rather than hidden. A monitor reports a great deal DisplCtrl will not
    ...
    public string UnofferedSummary
    {
        get
        {
            ...
            return $"{rest} more are reported but not offered: read-only facts, and codes specific to "
                 + "this manufacturer that DisplCtrl will not write blind. All of them, with their current "
