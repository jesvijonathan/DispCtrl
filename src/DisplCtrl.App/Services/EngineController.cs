...
{
    private const string ProcessName = "DisplCtrl.Engine";
    ...
    private static string? LocateEngine()
    {
        ...
        string beside = Path.Combine(appDir, "DisplCtrl.Engine.exe");
        if (File.Exists(beside)) return beside;

        // Dev layout: src/DisplCtrl.App/bin/<cfg>/<tfm>/<rid>/ -> src/DisplCtrl.Engine/bin/...
        var dir = new DirectoryInfo(appDir);
        for (int up = 0; up < 6 && dir is not null; up++, dir = dir.Parent)
        {
            if (!string.Equals(dir.Name, "DisplCtrl.App", StringComparison.OrdinalIgnoreCase)) continue;

            string relative = Path.GetRelativePath(dir.FullName, appDir);
            string candidate = Path.Combine(dir.Parent!.FullName, "DisplCtrl.Engine", relative, "DisplCtrl.Engine.exe");
