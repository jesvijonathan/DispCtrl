...
public sealed class PresetsViewModel : INotifyPropertyChanged
{
    ...
    /// </remarks>
    private readonly Func<DispCtrlSettings> _settingsSource;
    ...
    private DispCtrlSettings _settings => _settingsSource();

    public PresetsViewModel(Func<DispCtrlSettings> settings, Func<IReadOnlyList<DisplayInfo>> displays,
    ...
    {
        if (!DispCtrl.Core.FeatureFlags.Presets) return "Presets are disabled in this build.";
    ...
    private async Task CheckOnceAsync()
    {
        ...
        DispCtrlSettings settings = System.Text.Json.JsonSerializer.Deserialize(
            System.Text.Json.JsonSerializer.Serialize(_settings, SettingsJsonContext.Default.DispCtrlSettings),
