...
public static class QuickPanelCatalog
{
    ...
    [
        ...
        new("focus", "Focus", "\uE890",
            "Focus mode's switch, and how far it dims everything but the window in use.", false),
        new("oledCare", "OLED care", "\uE7EF",
            "OLED idle protection's switch, how dim it goes and how soon.", false),
        new("displays", "Displays", "\uE7F4",
            "A block for each display, with the rows chosen under Each display.", true),
        new("taskbar", "Taskbar", "\uE75A",
            "Transparency, the taskbar's glass and opacity, and auto-hide, in a block you can fold away.", false),
        new("presets", "Presets", "\uE768",
    ...
    [
        ...
        new("dimming", "Software dimming", "\uEC8A", "Dims below what the backlight can reach.", false),
        new("warmth", "Warmth", "\uF08C", "This display's own night light strength. Only shown while night light runs per display, because in unison or through Windows it would change nothing.", true),
        new("toggles", "Switches", "\uE8A9", "A strip of small switches for this display, chosen under Switches on each display.", true),
        new("resolution", "Resolution", "\uE9A6", "The display's resolution.", false),
        new("refresh", "Refresh rate", "\uE823", "How many times a second the display redraws.", false),
        new("scaling", "Scale", "\uE8A9", "How large Windows draws text and apps on this display.", false),
        new("orientation", "Orientation", "\uE8EB", "Landscape, portrait, or either flipped.", false),
        new("input", "Input source", "\uE8CB", "The monitor's input. Appears once the monitor has said what it supports.", false),
        new("controls", "Monitor controls", "\uE9E9", "Everything else the monitor lets you change over DDC/CI: contrast, volume, colour presets.", false),
    ];
    ...
    [
        ...
        new("vrr", "Variable refresh", "\uE9D9", "Variable refresh rate, where the display supports it.", false),
        new("adaptive", "Adaptive brightness", "\uE9F3", "Windows' automatic brightness, where the display has a sensor.", false),
        new("rotation", "Auto-rotate", "\uE7AD", "Rotate with the device, where it can sense which way up it is.", false),
        new("focusDimming", "Focus dimming", "\uE890", "Whether focus mode may dim this display.", false),
        new("oledProtection", "OLED care", "\uE7EF", "Idle protection for this display. OLED displays only.", false),
        new("rest", "Rest now", "\uEA14", "Rest this OLED display under the idle-protection dimming until the computer is next used. OLED displays only.", false),
        new("sleep", "Monitor sleep", "\uE7E8", "Switch this monitor off through its own power control after inactivity. External monitors only.", false),
...
public sealed class QuickPanelSettings
{
    ...
    /// </remarks>
    public static readonly string[] FoldedByDefault = ["oledCare", "focus", "displayMode", "taskbar", "nightLight"];
