...
public sealed class AwakeSettings
{
    ...
    /// <summary>Also turn each display's real backlight down, on those that allow it.</summary>
    /// <remarks>
    /// Only at <see cref="DisplaysOffPercent"/> 90 and above, where "off" is what
    /// was asked for; each backlight goes back to where it was as its display wakes.
    /// </remarks>
    public bool DisplaysOffBacklight { get; set; }

    /// <summary>The darkness from which the backlight may be turned down too.</summary>
    public const int BacklightFromPercent = 90;
