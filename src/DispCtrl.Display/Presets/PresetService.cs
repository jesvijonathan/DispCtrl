...
/// is exactly the set of calls DispCtrl.Display exists for — mode changes, CCD
...
public static class PresetService
{
    ...
    /// </summary>
    /// <remarks>
    /// Only the controls DispCtrl is willing to write. Recording one it would
    ...
    /// </para>
    /// </remarks>
    public static PresetResult Apply(Preset preset, IReadOnlyList<DisplayInfo> displays, DispCtrlSettings settings)
    {
        ...
        using var gate = new Mutex(false, @"Local\DispCtrl.PresetApply");
        bool acquired;
    ...
    private static PresetResult ApplyCore(Preset preset, DispCtrlSettings settings)
    {
    ...
    private static void ApplyBrightness(Preset preset, List<(DisplayInfo Display, PresetMonitor State)> matched,
                                        DispCtrlSettings settings, List<string> notes)
    {
    ...
    /// </remarks>
    private static void ApplyTaskbarBehaviour(Preset preset, DispCtrlSettings settings)
    {
    ...
    private static void ApplyTaskbar(Preset preset, List<(DisplayInfo Display, PresetMonitor State)> matched, DispCtrlSettings settings)
    {
        foreach ((DisplayInfo d, PresetMonitor m) in matched)
        {
            MonitorSettings ms = settings.For(d.Token);
