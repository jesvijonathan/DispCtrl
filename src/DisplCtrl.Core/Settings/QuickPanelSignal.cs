...
/// </summary>
/// <remarks>
/// The two DisplCtrl processes otherwise share only <c>settings.json</c>, and that
...
{
    private const string ShowName = @"Local\DisplCtrl.QuickPanel.Show";
    private const string AliveName = @"Local\DisplCtrl.QuickPanel.Alive";
    ...
    /// </summary>
    /// <remarks>
    /// Named mutexes, not a process lookup: several DisplCtrl.App processes can
    ...
    private static string? Sibling()
    {
        try
        {
            ...
            string candidate = System.IO.Path.Combine(here, "DisplCtrl.App.exe");
