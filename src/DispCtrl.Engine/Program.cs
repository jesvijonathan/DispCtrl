...
internal static class Program
{
    ...
    private static int Stop()
    {
        ...
        {
            Console.Error.WriteLine("no DispCtrl engine is running.");
            return 1;
        }
    ...
    private static int Run(TimeSpan? duration, bool trace)
    {
        ...
        // token may not carry.
        using var mutex = new Mutex(initiallyOwned: true, "Local\\DispCtrl.Engine", out bool isNew);
        if (!isNew)
        {
            Console.Error.WriteLine("another DispCtrl engine is already running.");
            return 0;
        }
        ...
        {
            cts.CancelAfter(d);
            Console.WriteLine($"DispCtrl engine running for {d.TotalSeconds:0}s, then restoring. Ctrl+C to stop early.");
        }
        else
        {
            Console.WriteLine("DispCtrl engine running. Ctrl+C to stop and restore.");
        }
