...
internal sealed class NightLightService : IDisposable
{
    ...
    public NightLightService(DispCtrlSettings settings)
    {
        _settings = settings;
    ...
    private bool Reconcile(DispCtrlSettings settings)
    {
    ...
    private bool ReconcileCore(DispCtrlSettings settings)
    {
    ...
    private void Tick()
    {
        try
        {
            ...
            // overrides all land in one place, in DispCtrl.Core, so the engine and
