...
internal sealed unsafe partial class FocusService : IDisposable
{
    ...
    private void Configure(bool rebuildMasks = true)
    {
        ...
        if (hooksChanged)
        {
            ...
            if (active)
            {
                ...
                if (prioritize) { AddHook(0x8001, 0x8003); AddHook(0x8017, 0x8017); } // destroy, show, hide; cloaked
            }
    ...
    private static void WindowEvent(nint hook, uint evt, nint window, int obj, int child, uint thread, uint time)
    {
        try
        {
            ...
            if (evt == 0x800B && !pointerMoved && (window != self._foreground || obj != 0)
                && !(window == self._automaticWindow && obj == 0)) return;
            if (pointerMoved)
            {
            ...
            if (evt is 0x8001 or 0x8003 or 0x8017 && window == self._automaticWindow)
            {
                self._automaticWindow = 0;
                self._automaticClass = "";
                self.Tick();
    ...
    private void Tick()
    {
        ...
        if (dimRequest is { } asked && _dimAppliedAt == 0)
        {
            if (DateTimeOffset.UtcNow >= asked.AddSeconds(Math.Clamp(awake.DisplaysOffDelaySeconds, 0, 30)))
            {
                ...
                if (awake.DisplaysOffBacklight && awake.DisplaysOffPercent >= AwakeSettings.BacklightFromPercent)
                    Power.DisplaysOffBacklight.Lower(_masks.Where(m => m.DimChosen).Select(m => m.Display));
        ...
        foreach (Mask mask in _masks)
        {
            ...
            {
                mask.DimWoken = true;
                Power.DisplaysOffBacklight.Restore(mask.Display.Token);
            }
    ...
    {
        Power.DisplaysOffBacklight.Restore();
    ...
    private static bool IsAutomaticCandidate(nint window, string cls)
    {
        ...
        // Seen, not merely existing: a Teams notification that had gone was
        // cloaked, not hidden or destroyed, and kept its cut-out in the dim
        // until the next click. DWMWA_CLOAKED is 14.
        if (IsWindowVisible(window) == 0) return false;
        if (DwmGetWindowAttributeInt(window, 14, out int cloaked, sizeof(int)) == 0 && cloaked != 0) return false;
        // Or slid off every screen, which is how some notifications leave. MONITOR_DEFAULTTONULL.
        if (MonitorFromWindow(window, 0) == 0) return false;
