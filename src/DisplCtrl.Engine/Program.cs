...
using DisplCtrl.Display.Cli;
using DisplCtrl.Engine.Color;
using DisplCtrl.Engine.Input;
using DisplCtrl.Engine.Presets;
using DisplCtrl.Engine.Taskbar;
using Windows.Win32;
...
internal static class Program
{
    ...
    /// </para>
    /// </remarks>
    private const string StopEventName = @"Local\DisplCtrl.Engine.Stop";
    ...
    private static int Stop()
    {
        ...
        {
            Console.Error.WriteLine("no DisplCtrl engine is running.");
            return 1;
        }
    ...
    private static int ListDisplays()
    {
        ...
        for (int i = 0; i < displays.Count; i++)
        {
            ...
            Console.WriteLine($"    taskbar     : {(hiding ? "HIDDEN by DisplCtrl" : "normal")}");
    ...
    private static int Run(TimeSpan? duration, bool trace)
    {
        ...
        // token may not carry.
        using var mutex = new Mutex(initiallyOwned: true, "Local\\DisplCtrl.Engine", out bool isNew);
        if (!isNew)
        {
            Console.Error.WriteLine("another DisplCtrl engine is already running.");
            return 0;
        }
        ...
        {
            cts.CancelAfter(d);
            Console.WriteLine($"DisplCtrl engine running for {d.TotalSeconds:0}s, then restoring. Ctrl+C to stop early.");
        }
        else
        {
            Console.WriteLine("DisplCtrl engine running. Ctrl+C to stop and restore.");
        }
